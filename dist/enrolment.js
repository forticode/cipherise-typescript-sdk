...
exports.EnrolmentResult = EnrolmentResult;
/**
...
class Enrolment {
     ...
     * @memberof Enrolment
     */
    constructor(service, baseLogger, logId, WaveCodeUrl, directEnrolUrl, statusUrl, validateUrl, username, publicKeys = new Map(), deviceId = "", confirmationUrl = "") {
    ...
    validate() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const data = yield this.service.getUrl(this.validateUrl);
                if (data.failReason) {
                    throw new Error(data.failReason);
                }
                this.logger.verbose(`Retrieved identicon URL: ${data.identiconURL}`, data);
                this.publicKeys = util.objectToPublicKeyMap(data.publicKeys);
                const deviceDetails = {
                    id: data.deviceId,
                    keys: data.publicKeys
                };
                this.logger.verbose(`Retrieved device details: ${JSON.stringify(deviceDetails)}`, data);
    ...
    confirm(success, payload) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (success &&
                ...
                request.payload = yield this.service.encryptPayloadData(this.publicKeys.get(1), payloadRequest);
            }
            // Send our request.
            this.logger.debug(`Enrolling with request: \`${JSON.stringify(request)}\``);
            const response = yield this.service.postUrl(this.confirmationUrl, request);
            this.logger.verbose(`Finished enrolment, response: ${JSON.stringify(response)}`);
            ...
            if (payload !== undefined && Object.keys(payload.set).length > 0) {
                const payloadResponseJson = this.service.decryptPayloadJson(this.publicKeys.get(1), response.payload);
                success = success && payloadResponseJson.setResponse;
                const payloadVerifyUrl = response["payloadVerifyURL"];
                if (payloadVerifyUrl) {
                    yield this.service.postUrl(payloadVerifyUrl, { verified: success });
                }
                payloadResponse = new payload_1.PayloadResponse();
 ...
 */
var EnrolmentState;
(function (EnrolmentState) {
    /**
     * The enrolment has been started.
     */
    EnrolmentState[EnrolmentState["Initialised"] = 0] = "Initialised";
    /**
     * The enrolment WaveCode has been scanned.
     */
    EnrolmentState[EnrolmentState["Scanned"] = 1] = "Scanned";
    /**
     * The service provider has received information about the device.
     */
    EnrolmentState[EnrolmentState["Validated"] = 2] = "Validated";
    /**
     * The service provider has confirmed whether or not the enrolment has succeeded.
     */
    EnrolmentState[EnrolmentState["Confirmed"] = 3] = "Confirmed";
    /**
     * The enrolment has failed.
     */
    EnrolmentState[EnrolmentState["Failed"] = 4] = "Failed";
    /**
     * Unknown state. Should never occur.
     */
    EnrolmentState[EnrolmentState["Unknown"] = 5] = "Unknown";
})(EnrolmentState = exports.EnrolmentState || (exports.EnrolmentState = {}));
//# sourceMappingURL=enrolment.js.map